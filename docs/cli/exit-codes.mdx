...
- `cxc emit` completed target-file generation with the selected plugin
...
- `cxc deploy` cannot complete the selected deployment stage
...
| `cxc check --json` | JSON contains valid diagnostics. | JSON contains validation or parse errors. |
...
| `cxc emit` | The target is found, IR lowering succeeds and target-file generation completes. | Target lookup, IR lowering, plugin compatibility, plugin startup or target-file generation fails. |
| `cxc deploy` | The selected deployment stage completes through the backend. | Backend lookup, lowering, plugin execution, provisioning, or event execution fails. |
...
`cxc check --json` writes its diagnostics to standard output.
Use both the JSON body and the process status:

```bash
if cxc check -i scenarios/HelloWorld.cradle --json > validation.json; then
