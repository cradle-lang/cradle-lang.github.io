...
| `cxc emit` | Generate target-specific files through a backend plugin without deploying. |
| `cxc deploy` | Run deployment stages through a selected backend. |
...
:::important
`cxc emit` generates target-specific files. CradleXC does not automatically
execute those files.
:::
...
Runs the selected backend's deployment workflow. Deployment stages are
backend-dependent and can be limited with `--stage`.

```bash
cxc deploy \
  -i <scenario.cradle> \
  --backend <backend> \
  --stage <all|compile|render|boot|provision|events>
```

Use `cxc emit` when you only need generated files. Deployment can provision
systems or run events, depending on the selected backend and stage.
