...
| `cxc configs list` | List configuration implementations available to the compiler. |
| `cxc heuristics list` | List heuristic implementations available to the compiler. |
...
Omit `-o` to write the compiled YAML to standard output.
...
List the content implementations available from the configured sources:
...
Use `--config-dir <path>` or `--heuristics-dir <path>` to inspect a specific
local content directory. These commands inspect available content; they do
not generate target files or deploy an environment.
