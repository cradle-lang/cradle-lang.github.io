...
By default, `cxc plugin install <plugin_name>` installs into:
...
A discovered backend can be selected through `cxc emit` or `cxc deploy`.
...
A backend can be discovered successfully while `cxc emit` still fails.
