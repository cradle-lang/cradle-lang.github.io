...
<div className="cradle-lifecycle">
  ...
  <div className="cradle-lifecycle-step">
    ...
    <div className="cradle-lifecycle-content">
      ...
      <span>
        Define systems, networks, objects and events in a <code>.cradle</code> scenario.
      </span>
    </div>
  </div>
...
CRADLE and CradleXC remain independent of target-specific deployment logic.
Backend plugins handle that translation for each supported target.
...
`cxc emit` uses a compatible backend plugin to render target-specific files from a CRADLE scenario.
...
- Tool developers can consult the [Deployment IR contract](../backend-development/deployment-ir-contract).
