...
<div className="cxc-command-grid">
  ...
  <div className="cxc-command-card">
    <h3>Import and migrate</h3>
    <div className="cxc-command-description">
      Bring external or earlier scenario definitions into the current workflow.
    </div>
...
`cxc emit` uses a compatible backend plugin to generate target-specific files from a CRADLE scenario.
...
`cxc import` and `cxc migrate` support workflows for importing supported external definitions and migrating earlier CRADLE scenarios to the current language generation.

The exact behavior, arguments and available options for each command are documented in the CLI reference.
