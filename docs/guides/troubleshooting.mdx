...
If no path is returned, install CradleXC from the upstream source repository:
...
If the selected plugin has been published to the CRADLE release repository, it
...
If no release is available for that plugin, build the plugin from its own
source repository and place the resulting `cxc-backend-<name>` executable on
`PATH`. Plugin installation currently provides Linux x86_64 binaries and
...
If you need to update CradleXC, pull the upstream source and reinstall it:
