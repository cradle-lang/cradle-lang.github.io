...
The event name is `1`. It should have a matching event definition.
...
CRADLE can express dependencies through `waitfor()` where supported by the current compiler behavior.
...
<div
  ...
  role="img"
  aria-label="Event 2 waits for event 1"
>
  <div className="scenarioTopologyValue">
    <code>event 1</code>
  </div>
...
:::note
The current JSON Schema and compiler behavior do not represent `waitfor` in exactly the same way. Validate dependency behavior against the CradleXC release you are using.
:::
