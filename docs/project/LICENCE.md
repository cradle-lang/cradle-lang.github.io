# CRADLE Proprietary Software Licence

Version 1.1

Effective date: 25 August 2026

Copyright © 2026 National Cybersecurity R&D Lab (NCL). All rights reserved.

IMPORTANT: PLEASE READ THIS LICENCE CAREFULLY.

By downloading, installing, accessing or using the Software, the Licensee agrees to be bound by this Licence.

If an individual accepts this Licence on behalf of an organisation, that individual represents that they have authority to bind that organisation.

If the Licensee does not agree to this Licence, the Licensee must not download, install, access or use the Software.

## 1. Definitions

For the purposes of this Licence:

"Authorised Purpose" means:

- non-commercial academic research
- teaching and educational activities
- internal testing and evaluation
- development and testing of independent Plugins through documented CRADLE Interfaces
- authorised cyber-range, laboratory or testbed activities conducted for any of the purposes above

"Authorised System" means a workstation, server, laboratory system, cyber range, testbed, authorised CI/CD environment or authorised cloud environment on which the Licensee is permitted to install or execute the Software.

"Authorised User" means an individual authorised by the Licensee to access or use the Software for an Authorised Purpose and within the scope of this Licence.

"Commercial Use" means use primarily intended for commercial advantage or monetary compensation, including:

- production deployment
- enterprise operations other than internal evaluation
- paid consultancy or training
- commercial service provision
- managed services
- customer-facing services
- resale
- rental or leasing
- sublicensing
- commercial redistribution

"CRADLE" means the CRADLE software project.

"CRADLE Interface" means CRADLE language syntax, a documented CRADLE Intermediate Representation, API, plugin interface, extension interface, schema or other interoperability mechanism expressly made available by NCL for authorised use.

"Documentation" means the user guides, reference materials, specifications and other documentation supplied by NCL for use with the Software.

"Generated Output" means configuration files, deployment files, scripts, data or other materials generated through authorised use of the Software.

"Licence" means this CRADLE Proprietary Software Licence.

"Licence Materials" means:

- this Licence
- an applicable licence schedule
- the CRADLE Copyright Notice
- release-specific terms
- an applicable `NOTICE` file
- any other document expressly incorporated into this Licence

"Licensee" means the individual or organisation that accepts this Licence and is authorised to use the Software.

"Licensor" means National Cybersecurity R&D Lab (NCL), or another person or entity expressly identified as having authority to license the relevant Software.

"NCL" means National Cybersecurity R&D Lab.

"Plugin" means software developed independently of the proprietary CRADLE compiler and assembler that interoperates with CRADLE through a documented CRADLE Interface.

"Plugin Owner" means the author, copyright owner or other person or entity possessing sufficient rights to license a Plugin.

"Release" means a particular version or distribution of CRADLE.

"Software" means the proprietary CRADLE compiler, assembler and any other proprietary executable, implementation material or CRADLE-owned component supplied under this Licence.

## 2. Ownership

The Software is licensed, not sold.

All copyright and other rights, title and interest in the proprietary CRADLE compiler, assembler and associated proprietary materials remain with the Licensor or respective rights holder.

Nothing in this Licence transfers ownership of:

- the proprietary CRADLE Software
- non-public CRADLE source code
- CRADLE internal implementation materials
- CRADLE-owned documentation or development materials
- any other intellectual property owned or controlled by the Licensor

All rights not expressly granted under this Licence are reserved.

The copyright framework applicable to CRADLE is described in [`COPYRIGHT.md`](./COPYRIGHT.md).

## 3. Licence Grant

Subject to the Licensee's continuing compliance with this Licence, NCL grants the Licensee a limited, revocable, non-exclusive, non-transferable and non-sublicensable licence to:

- install the Software on Authorised Systems
- execute and use the Software solely for an Authorised Purpose
- create, edit, validate, compile, assemble and process `.cradle` files
- use documented CRADLE Interfaces for authorised interoperability
- develop and test independent Plugins through documented CRADLE Interfaces
- use Generated Output as permitted by this Licence
- make copies reasonably necessary for authorised installation, backup and recovery
- reproduce Documentation internally for Authorised Users where reasonably necessary to use the Software

No right to access or obtain non-public CRADLE source code is granted unless expressly provided in a separate written agreement.

No right is granted except as expressly stated in this Licence, another applicable written agreement or applicable law.

## 4. Authorised Users

The Software may be used only by the Licensee and its Authorised Users within the scope of the applicable licence arrangement.

The Licensee is responsible for ensuring that individuals given access to the Software are authorised to use it.

The Licensee must take reasonable measures to prevent access by unauthorised persons.

## 5. Authorised Systems

Subject to the applicable licence arrangement, authorised use may include installation and execution on:

- user workstations
- laboratory infrastructure
- cyber-range systems
- servers
- authorised CI/CD infrastructure
- authorised cloud environments
- authorised testbeds

Any limits concerning users, machines, installations, servers, environments or testbeds shall be determined by the applicable licence schedule or written agreement.

## 6. Internal Use

Subject to any applicable licence restrictions, the Licensee may use CRADLE for authorised internal purposes.

Internal use does not, by itself, grant permission to:

- provide copies of CRADLE to customers
- redistribute CRADLE outside the licensed organisation
- sublicense CRADLE
- resell CRADLE
- make CRADLE available as a separately distributed product

## 7. Research, Education and Evaluation

The Licensee may use the Software for:

- non-commercial academic research
- teaching and educational activities
- internal testing
- internal technical evaluation
- authorised cyber-range, laboratory and testbed experiments
- development and testing of independent Plugins

The Licensee may publish research findings and benchmark results obtained through authorised use, provided that the publication:

- does not disclose confidential or non-public CRADLE information
- does not distribute the proprietary Software
- does not misrepresent the methodology or results
- does not imply NCL endorsement
- complies with applicable research, ethical and legal requirements

## 8. Commercial and Production Use

This Licence does not permit Commercial Use or production deployment.

Without NCL's prior written authorisation, the Licensee must not use the Software for:

- production operations
- enterprise deployment other than internal evaluation
- paid consultancy or training
- commercial service provision
- managed services
- customer-facing services
- resale
- sublicensing
- rental or leasing
- commercial redistribution
- providing access to the Software for the benefit of an unauthorised third party

Commercial or production use requires a separate written agreement or licence schedule issued or approved by NCL.

## 9. Restrictions

Except as expressly authorised under this Licence, another written agreement or applicable law, the Licensee must not:

- copy or reproduce the proprietary Software except as reasonably necessary for authorised installation, use or backup
- modify, patch or alter the proprietary compiler or assembler
- distribute or make proprietary CRADLE Software available to an unauthorised third party
- sublicense the proprietary CRADLE Software
- resell the proprietary CRADLE Software
- rent or lease the proprietary CRADLE Software
- commercially redistribute the proprietary CRADLE Software without authorisation
- publish proprietary CRADLE Software on a public repository, website or file-sharing service
- remove, conceal or alter copyright, proprietary, attribution or legal notices
- disclose non-public CRADLE source code or confidential implementation information
- circumvent technical access controls applicable to proprietary CRADLE Software
- represent an unauthorised modification, Plugin, integration or distribution as an official CRADLE product
- use CRADLE in a manner that exceeds the scope of the applicable licence arrangement

## 10. Copies and Backups

The Licensee may make copies of the Software only to the extent reasonably necessary for authorised installation, execution, backup or recovery, subject to the applicable licence arrangement and applicable law.

Any permitted copy must retain applicable copyright, proprietary and legal notices.

## 11. Reverse Engineering and Mandatory Computer-Program Rights

Except to the extent expressly permitted by applicable law, the Licensee must not:

- reverse engineer the Software
- decompile the Software
- disassemble the Software
- attempt to derive non-public source code or implementation information
- use information obtained through unauthorised analysis to reproduce proprietary CRADLE implementation materials

Nothing in this Licence excludes or restricts a permitted use or statutory right that cannot lawfully be excluded or restricted.

This section is intended to operate subject to sections 238 to 240 of the Copyright Act 2021 of Singapore and any other applicable mandatory provisions.

## 12. CRADLE Interfaces and Interoperability

Subject to this Licence, the Licensee may use documented CRADLE Interfaces for the purpose of developing independent software that interoperates with CRADLE.

A documented CRADLE Interface may include:

- CRADLE language syntax
- documented Intermediate Representation formats
- documented application programming interfaces
- plugin interfaces
- extension interfaces
- schemas
- documented testbed integration mechanisms

Permission to use a documented CRADLE Interface does not grant:

- ownership of CRADLE
- access to non-public source code
- permission to redistribute proprietary CRADLE binaries
- rights in confidential CRADLE implementation information
- permission to circumvent access controls
- a right to represent independent software as an official CRADLE component

## 13. Independent Plugin Ownership

Copyright and other rights in a Plugin developed independently of the proprietary CRADLE compiler and assembler remain with the Plugin Owner unless otherwise agreed in writing.

A Plugin does not become the property of the Licensor merely because it:

- uses CRADLE syntax
- consumes a documented CRADLE Intermediate Representation
- produces a documented CRADLE Intermediate Representation where permitted
- communicates through a documented CRADLE API
- uses a documented plugin interface
- integrates CRADLE with a testbed or other system
- uses Generated Output

The Licensor does not acquire copyright ownership of independently developed Plugin code solely by reason of interoperability with CRADLE.

## 14. Independent Plugin Licensing

The Plugin Owner determines the licence applicable to independently developed Plugin code.

Subject to any separate licence obligations applicable to CRADLE-owned or third-party material incorporated into the Plugin, the Plugin Owner may distribute the Plugin under:

- an open-source licence
- a proprietary licence
- a commercial licence
- an internal-use licence
- another lawful licence selected by the Plugin Owner

For example, independently developed Plugin code may be licensed under MIT, Apache License 2.0, Mozilla Public License 2.0, GPL, BSD or proprietary terms where selected by the Plugin Owner.

The proprietary CRADLE licence does not, solely because a Plugin interoperates with CRADLE, require independently developed Plugin code to be licensed under this Licence.

## 15. Commercial Plugins

Nothing in this Licence prohibits a Plugin Owner from commercially licensing or distributing independently developed Plugin code solely because that Plugin interoperates with CRADLE.

Commercial distribution of a Plugin does not grant the Plugin Owner or its customers a right to:

- redistribute the proprietary CRADLE compiler
- redistribute the proprietary CRADLE assembler
- sublicense proprietary CRADLE Software
- resell proprietary CRADLE Software
- otherwise exercise rights in CRADLE that have not been separately granted

A user of a commercial Plugin remains responsible for obtaining any CRADLE licence required for use of the proprietary CRADLE Software.

## 16. CRADLE SDK and Development Materials

Where a CRADLE SDK, library, schema, template or other development material is supplied under this Licence, that material remains proprietary unless a separate licence expressly applies.

The Licensee may use such development material for Plugin development only within the scope expressly authorised.

Use of a proprietary CRADLE SDK does not, by itself, transfer ownership of independently authored Plugin code to the Licensor.

Any CRADLE-owned source code copied, incorporated, modified or derived from the SDK remains subject to its applicable licence.

## 17. User-Created `.cradle` Files

Unless otherwise agreed in writing, copyright and other rights in independently authored `.cradle` files remain with their respective authors or rights holders.

Use of the Software to create, edit, validate, compile, assemble or otherwise process a `.cradle` file does not, by itself, transfer ownership of the file to the Licensor.

This Licence does not, solely because a `.cradle` file is processed by CRADLE, become the licence governing independently authored content in that file.

## 18. Generated Output

Unless otherwise provided under a separate written agreement, the Licensee may use, modify and distribute Generated Output produced through authorised use of CRADLE.

Use of CRADLE does not, by itself, cause Generated Output to become proprietary CRADLE Software.

Where Generated Output contains CRADLE-owned code, templates, libraries, runtime components or third-party licensed materials, those portions remain subject to their applicable licences.

## 19. Third-Party Plugins

Unless expressly stated otherwise, an independently developed third-party Plugin is not developed, maintained, supported, certified, approved or endorsed by the Licensor.

The Plugin Owner is responsible for matters relating to its Plugin, including:

- copyright ownership
- licensing
- functionality
- security
- compatibility
- maintenance
- support
- third-party dependencies
- compliance with applicable law

To the maximum extent permitted by applicable law, the Licensor is not responsible for the functionality, security, licensing, compatibility or conduct of independently developed Plugins unless expressly agreed otherwise.

## 20. Third-Party Software

The Software may contain, depend upon, link to, communicate with or otherwise interact with third-party software.

Third-party components remain subject to their respective copyright notices and licence terms.

Nothing in this Licence modifies, supersedes or overrides an applicable third-party licence.

The Licensee is responsible for complying with applicable third-party terms arising from its own use, modification or distribution of third-party software.

## 21. Confidentiality

"Confidential Information" means non-public CRADLE source code, credentials, implementation documentation, security information, development materials and other information identified as confidential or which a reasonable person would understand to be confidential from the circumstances of disclosure.

Confidential Information does not include information that the Licensee demonstrates:

- is lawfully public through no breach of an obligation
- was lawfully known without restriction before disclosure
- was independently developed without use of Confidential Information
- was lawfully received from a third party without a confidentiality restriction

The Licensee must:

- use Confidential Information only for the Authorised Purpose
- protect it using at least reasonable care
- disclose it only to Authorised Users who need access and are subject to appropriate confidentiality obligations
- notify NCL promptly of any known unauthorised access or disclosure

Where disclosure is required by law, the Licensee must, where legally permitted, give NCL prompt notice and disclose only the information legally required.

On request or termination, the Licensee must return or securely destroy Confidential Information, except where retention is required by law.

These confidentiality obligations survive termination for as long as the information remains confidential.

## 22. Notices

The Licensee must not remove, conceal or alter:

- copyright notices
- proprietary notices
- attribution notices
- licence notices
- third-party notices

included with the Software.

## 23. CRADLE Name and Branding

This Licence does not grant a licence to any CRADLE or NCL trade mark, logo or other branding except to the extent necessary to make truthful statements concerning compatibility or use.

Subject to applicable branding guidelines, an independent developer may make truthful statements such as:

- "compatible with CRADLE"
- "plugin for CRADLE"
- "designed to integrate with CRADLE"

The following or similar statements may not be used without express written authorisation:

- "Official CRADLE Plugin"
- "CRADLE Certified"
- "CRADLE Approved"
- "NCL Approved"

Nothing in this provision prevents nominative or other lawful use that cannot validly be prohibited.

## 24. Misuse of CRADLE

CRADLE is provided as a general-purpose software development, modelling, simulation and cyber-range technology.

The availability of CRADLE, or of any feature, interface, example, configuration, Plugin mechanism or other capability, does not constitute authorisation to use CRADLE against any system, network, service, device, account, data or infrastructure without lawful authority.

The Licensee is solely responsible for ensuring that its use of CRADLE is lawful and properly authorised.

The Licensee must not use CRADLE for:

- unlawful activity
- malicious activity
- fraudulent activity
- unauthorised access
- unauthorised interference with systems or networks
- unauthorised disruption, damage or compromise of systems, services or data
- any other activity prohibited by applicable law or the applicable CRADLE licence arrangement

NCL does not authorise, direct, encourage or endorse the use of CRADLE for unlawful, malicious, fraudulent, abusive or otherwise unauthorised purposes.

To the maximum extent permitted by applicable law, NCL and the applicable CRADLE copyright holders, licensors, developers, contributors, maintainers and distributors shall not be responsible or liable for any act, omission, loss, damage, claim, investigation, prosecution, penalty, proceeding, cost or other consequence arising from or relating to a user's unlawful, malicious, abusive or unauthorised use of CRADLE.

A user's misuse of CRADLE is undertaken independently of NCL and does not, by itself, create any agency, partnership, employment, endorsement or other relationship between that user and NCL.

Nothing in this section excludes or limits any liability that cannot lawfully be excluded or limited under applicable law.

## 25. Suspension for Misuse

The Licensor may suspend or restrict access to the proprietary CRADLE Software where the Licensor reasonably determines that the Licensee or an Authorised User is using, or permitting the use of, CRADLE for:

- unlawful purposes
- malicious purposes
- fraudulent purposes
- abusive purposes
- unauthorised access to systems or data
- activity materially outside the scope of the applicable licence

Any suspension under this section does not prevent the Licensor from exercising any other right available under this Licence, another applicable agreement or applicable law.

## 26. Updates and Support

Unless expressly provided under another written agreement, this Licence does not entitle the Licensee to:

- technical support
- maintenance
- updates
- upgrades
- new functionality
- future CRADLE releases

## 27. Future Releases

Future CRADLE Releases may be distributed under the same or different licence terms.

The licence applicable to each Release shall be the licence expressly identified for that Release.

Nothing in this Licence obliges the Licensor to:

- continue distributing CRADLE
- continue using this Licence for future Releases
- distribute future source code
- adopt an open-source licence
- maintain a particular licensing model

A change to the licence applicable to a future Release does not, by itself, amend, revoke or replace this Licence as it applies to an earlier Release.

## 28. Termination

NCL may terminate this Licence by written notice if the Licensee materially breaches it and fails to remedy a remediable breach within 14 days after receiving notice.

NCL may terminate this Licence immediately where the Licensee:

- unlawfully distributes or discloses the Software
- materially breaches confidentiality obligations
- circumvents technical access controls
- uses the Software for unlawful, malicious, fraudulent or unauthorised activity
- commits another material breach that cannot reasonably be remedied

Upon termination, the Licensee must:

- immediately cease using the Software
- delete or return all copies within its possession or control
- ensure that its Authorised Users cease using the Software
- confirm deletion or return in writing if reasonably requested

The Licensee may retain a copy only where required by applicable law. A retained copy must not be used and remains subject to this Licence.

Termination does not affect rights or liabilities accrued before termination.

## 29. Effect of Termination on Independent Materials

Termination of the Licensee's CRADLE licence does not, by itself, transfer ownership of:

- independently developed Plugin code
- independently authored `.cradle` files
- independently owned Generated Output

to the Licensor.

However, termination may prevent continued use of CRADLE itself where a valid CRADLE licence is required to use those materials with CRADLE.

## 30. Disclaimer of Warranty

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, THE SOFTWARE AND DOCUMENTATION ARE PROVIDED "AS IS" AND "AS AVAILABLE".

NCL DISCLAIMS ALL EXPRESS, IMPLIED AND STATUTORY WARRANTIES, REPRESENTATIONS AND CONDITIONS TO THE MAXIMUM EXTENT PERMITTED BY LAW, INCLUDING WARRANTIES OF MERCHANTABILITY, SATISFACTORY QUALITY, FITNESS FOR A PARTICULAR PURPOSE, ACCURACY, NON-INFRINGEMENT AND SECURITY.

NCL DOES NOT WARRANT THAT THE SOFTWARE WILL:

- BE ERROR-FREE OR UNINTERRUPTED
- BE FREE FROM VULNERABILITIES
- IDENTIFY OR PREVENT EVERY SECURITY ISSUE
- MEET THE LICENSEE'S REQUIREMENTS
- PRODUCE COMPLETE OR ERROR-FREE OUTPUT
- REMAIN COMPATIBLE WITH THIRD-PARTY SOFTWARE, PLUGINS OR INFRASTRUCTURE
- BE SUITABLE FOR PRODUCTION, SAFETY-CRITICAL OR MISSION-CRITICAL USE

THE LICENSEE IS RESPONSIBLE FOR REVIEWING AND TESTING ALL CONFIGURATIONS, SCRIPTS AND GENERATED OUTPUT BEFORE USE.

Nothing in this section excludes any warranty, right or liability that cannot lawfully be excluded.

## 31. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, NCL SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, EXEMPLARY, PUNITIVE OR CONSEQUENTIAL LOSS ARISING OUT OF OR IN CONNECTION WITH THE SOFTWARE, DOCUMENTATION, GENERATED OUTPUT OR THIS LICENCE.

THIS INCLUDES LOSS OF PROFITS, REVENUE, BUSINESS, OPPORTUNITY, GOODWILL, DATA OR USE, AND INTERRUPTION OF OPERATIONS.

TO THE MAXIMUM EXTENT PERMITTED BY LAW, NCL'S TOTAL AGGREGATE LIABILITY ARISING OUT OF OR IN CONNECTION WITH THIS LICENCE SHALL NOT EXCEED:

- THE TOTAL AMOUNT PAID BY THE LICENSEE TO NCL FOR THE RELEVANT SOFTWARE DURING THE 12 MONTHS BEFORE THE EVENT GIVING RISE TO LIABILITY

WHICHEVER APPLIES.

The limitations in this section apply regardless of the legal basis of the claim and even where a party was advised that loss was possible.

Nothing in this Licence excludes or limits liability where exclusion or limitation is prohibited by applicable law.

## 32. Indemnity for Misuse

To the extent permitted by applicable law, the Licensee shall indemnify NCL against third-party claims, losses, liabilities, penalties and reasonable costs arising directly from:

- the Licensee's unlawful, malicious, fraudulent or unauthorised use of CRADLE
- the Licensee's unauthorised distribution of the Software
- the Licensee's material breach of confidentiality
- content, data or materials supplied by the Licensee that infringe third-party rights

This section does not require the Licensee to indemnify NCL for loss caused by NCL's own fraud, wilful misconduct or liability that cannot lawfully be excluded.

## 33. Compliance with Law

The Licensee is responsible for complying with laws and regulations applicable to its use of CRADLE.

Nothing in this Licence requires either party to act in violation of applicable law.

## 34. No Implied Licence

No licence, permission or other right arises by implication, estoppel or otherwise merely because:

- CRADLE software has been provided
- a binary is publicly downloadable
- documentation is published
- an interface is documented
- interoperability is technically possible

Only rights expressly granted under this Licence, another applicable written agreement or applicable law are granted.

## 35. Reservation of Rights

Except for the rights expressly granted by this Licence, all rights in the proprietary CRADLE Software are reserved.

This reservation of rights does not claim copyright ownership of independently developed Plugins, independently authored `.cradle` files or independently owned Generated Output merely because those materials are used with CRADLE.

## 36. No Waiver

Failure by the Licensor to exercise or enforce a right under this Licence does not constitute a waiver of that right.

A waiver is effective only to the extent expressly provided by the party granting it.

## 37. Severability

If a provision of this Licence is held invalid, unlawful or unenforceable, that provision shall apply only to the maximum extent permitted by law.

The remaining provisions shall continue in effect to the extent legally possible.

## 38. Relationship with Other Agreements

This Licence may be supplemented by:

- a licence schedule
- research agreement
- academic agreement
- enterprise agreement
- commercial agreement
- order form
- other written agreement

Where a separately executed written agreement expressly conflicts with this Licence, that agreement shall prevail to the extent of the conflict in relation to the relevant Software or use.

## 39. Release-Specific Terms

A particular CRADLE Release or component may contain additional or different legal terms.

Where a Release contains or references a specific licence, `NOTICE` file, copyright notice or other expressly applicable terms, those terms govern the relevant Release or component.

## 40. Governing Law and Jurisdiction

This Licence is governed by the laws of the Republic of Singapore.

The Licensee and NCL irrevocably submit to the exclusive jurisdiction of the courts of Singapore in relation to any dispute arising out of or in connection with this Licence, the Software or its use.

A separately executed written agreement may specify different governing-law or dispute-resolution terms for the arrangement governed by that agreement.

Nothing in this Licence excludes the operation of a mandatory provision of applicable law.

## 41. Mandatory Rights

Nothing in this Licence excludes, restricts or waives any right, defence, exception, permitted use or protection that cannot lawfully be excluded, restricted or waived.

If a provision is inconsistent with a mandatory rule, that provision applies only to the maximum extent permitted by law.

This Licence is intended to operate subject to sections 238 to 240 of the Copyright Act 2021 of Singapore and any other applicable mandatory provisions.

## 42. Contact

Licensing and permission enquiries may be directed to:

**National Cybersecurity R&D Lab (NCL)**
Website: `https://ncl.sg/`

Any designated CRADLE licensing contact may also be identified in the relevant CRADLE distribution or project documentation.

## Legal Notices

A legal notice under this Licence must be in writing.

Notices to NCL must be sent to the legal or licensing contact identified in the applicable licence schedule, Release or CRADLE project documentation.

A notice is treated as received:

- when delivered personally
- two business days after dispatch by registered post
- when electronic delivery is acknowledged by the receiving party

This section does not govern copyright, attribution or proprietary notices embedded in the Software.

## Assignment

The Licensee must not assign, transfer, delegate or otherwise dispose of this Licence or any right under it without NCL's prior written consent.

NCL may assign or transfer this Licence together with the relevant Software or intellectual property rights to a successor, affiliate or entity acquiring or administering the relevant CRADLE rights.

Any attempted assignment contrary to this section is ineffective to the extent permitted by law.

## Survival

The provisions concerning ownership, restrictions, confidentiality, termination, disclaimers, limitation of liability, indemnity, governing law, jurisdiction and accrued rights survive termination or expiry to the extent necessary to give them effect.

## Entire Agreement and Order of Precedence

This Licence and the applicable Licence Materials constitute the agreement governing the Licensee's use of the relevant Software, unless a separately executed written agreement applies.

In the event of an inconsistency, the following order of precedence applies:

1. a separately executed written agreement
2. an applicable licence schedule or order form
3. release-specific licence terms
4. this Licence
5. an applicable `NOTICE` file
6. the Copyright Notice
7. the general Legal Notice
8. the Documentation

A higher-ranking document prevails only to the extent of the inconsistency.

No amendment or waiver is effective unless made in writing by a person authorised to bind the party against whom it is asserted.

## Third-Party Rights

Except for a person expressly identified in this Licence as entitled to enforce a particular provision, a person who is not a party to this Licence has no right under the Contracts (Rights of Third Parties) Act 2001 of Singapore to enforce any term of this Licence.

NCL's applicable copyright holders, licensors, developers, contributors, maintainers and distributors may rely on and enforce provisions that expressly protect them, including the warranty disclaimers and limitations of liability.

---

Copyright © 2026 National Cybersecurity R&D Lab (NCL). All rights reserved.

CRADLE is proprietary software.

Licensed under the CRADLE Proprietary Software Licence.
