...
| Property | `identifier("value")` | Assigns one or more values to a block. |
...
| Comment | `// comment text` | Adds a comment to the end of the line. |
...
CRADLE values are written as double-quoted strings.
...
Most values in CRADLE source are represented as quoted strings.
...
The grammar treats these as quoted arguments.

The compiler determines how a particular property interprets its values.

## Boolean-like values

Some CRADLE properties use the strings:
...
These values are written as quoted arguments in CRADLE source.

## Numeric values

Values such as durations are also represented as quoted arguments.
...
The compiler interprets the value according to the property being processed.
...
Some properties accept a parameter string as an additional argument.
...
Where multiple parameters are represented inside a single argument, they can use semicolon-separated `key=value` pairs when supported by that property.
...
The interpretation of parameter strings depends on the property using them.
