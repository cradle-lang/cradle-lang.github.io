...
<div className="architecture-flow">
  ...
  <div>event("collect_evidence") depends on event("initialize_client")</div>
...
The supported flow type is:

```text
sync
```

Use `executionFlow("sync")` when the property must be stated explicitly. Synchronous execution is also the default when the property is omitted.

:::note
Asynchronous execution is not currently supported by the Vagrant deployment path.
:::
