...
This declares an object named:

```text
InitializationScript
```

The declaration does not define where the artifact is located.

That is done in a separate object definition.
...
matches the declaration in `metadata()`.
...
```CRADLE
object("InitializationScript") >
    location("https://example.com/repository/scripts/initialize.sh").
```
...
object("CollectEvidence") >
    location("https://example.com/repository/scripts/collect-evidence.sh").
```
...
The object is then associated with the application instance:
...
Each event references the same object:
...
<div className="architecture-flow">
  ...
  <div>instance("HelloWorld") associates the object</div>

  <span>↓</span>

  <div>Events use runObject("HelloWorld", "")</div>

</div>

:::note
The public Hello World example uses `https://artifacts.example.org` as a placeholder repository. It does not host the example artifact.
:::
