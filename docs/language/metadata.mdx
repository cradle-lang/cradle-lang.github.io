...
| `eventType` | `sequence` or `DAG` | Schema required | Selects a sequential or dependency-graph event model. CradleXC defaults to `sequence` when omitted. |
...
A dependency-graph event model uses:

```CRADLE
eventType("DAG")
```

This allows event relationships to be expressed through dependencies.

Detailed event behavior is covered in [Events and Dependencies](./events-and-dependencies).
