...
CRADLE property values are written as quoted strings.
...
`waitfor()` can reference another event in compiler-supported dependency behavior.
...
:::note
The current schema and compiler behavior do not represent `waitfor` in exactly the same way. Validate dependency behavior against the CradleXC release being used.
:::
...
- `config`, `io`, `description`, `heuristic` and `executionFlow` include compiler-specific behavior
- `waitfor` differs between the schema and existing compiler behavior
