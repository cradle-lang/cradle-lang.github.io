...
- Follow with the release date using the exact supplied `Release date` value.
  This value is derived from the requested Git tag. For an annotated tag, it
  represents the tagger date. For a lightweight tag, it represents the date of
  the commit referenced by the tag. Do not replace it with the current date or
  workflow execution date. Use the same italicized date format and
...
- The capture already normalizes machine-specific paths, usernames and terminal
  color. Keep normalized or illustrative paths, remove accidental duplicate
