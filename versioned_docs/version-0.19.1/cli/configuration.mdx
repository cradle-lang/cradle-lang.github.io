...
Generation can layer additional local content directories on top of the primary
directory. A name found in the primary directory takes precedence; an extra
directory only fills in names that are not already present:
...
`dataset_dir` is the controller-side base directory for extraction output such
as network captures, node logs, and memory dumps. Its default is
`/opt/cxc/dataset`; override it with `CXC_DATASET_DIR` or a backend-specific
...
Package installation also generates shell completion scripts. Use the hidden
`cxc completions <shell>` command when installing or maintaining completions
manually.
