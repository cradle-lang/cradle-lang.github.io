...
:::note
`cxc build` is a hard rename of `cxc emit`; there is no `emit` alias. The
rendering purpose and command options are otherwise the same. The default
output directory changed from `./emit_<target>` to `./build_<target>`, and the
render state files changed from `.cxc-deploy.json` and `.cxc-deploy-ir.json` to
`.cxc-build.json` and `.cxc-build-ir.json`.
...
successful build. Output created by the former `cxc emit` state format is not a
valid baseline; run `cxc build` once to create current build state.
